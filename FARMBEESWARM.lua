...
luascripts:Element("Button", "Desync",nil, function()
	local ReplicatedStorage = game:GetService("ReplicatedStorage");
...
luascripts:Element("Button", "god mode",nil, function()
...
luascripts:Element("Button", "inf heath", nil, function()
	...
	LocalPlayer.Character.Humanoid:GetPropertyChangedSignal("Health"):Connect(function()
		LocalPlayer.Character.Humanoid.Health = 666
	end)
end)
...
--[[retard:Element("Slider", "ForwardTrack x", {min = 0, max = 100, default = 0})
retard:Element("Slider", "ForwardTrack y", {min = 0, max = 100, default = 0})
retard:Element("Slider", "ForwardTrack z", {min = -100, max = 100, default = 0})
		...
		if j ~= game.Players.LocalPlayer and j.Team ~= game.Players.LocalPlayer.Team  then
			pcall(function()
				...
				if ( b.x ~= a.x ) and ( b.z ~= a.z ) then
					if ( b.x > a.x ) and ( b.z > a.z ) then
						j.Character.LeftUpperLeg.LeftHip.C0 = CFrame.new(values.different.retard["ForwardTrack x"].Slider, values.different.retard["ForwardTrack y"].Slider, values.different.retard["ForwardTrack z"].Slider)
					elseif ( b.x < a.x ) and ( b.z < a.z ) then
						j.Character.RightUpperLeg.RightHip.C0 = CFrame.new(values.different.retard["ForwardTrack x"].Slider/-1, values.different.retard["ForwardTrack y"].Slider, values.different.retard["ForwardTrack z"].Slider)
					elseif ( b.x > a.x ) and ( b.z < a.z ) then 
						j.Character.UpperTorso.Waist.C0 = CFrame.new(values.different.retard["ForwardTrack x"].Slider, values.different.retard["ForwardTrack y"].Slider, values.different.retard["ForwardTrack z"].Slider)
					else
						j.Character.UpperTorso.Waist.C0 = CFrame.new(values.different.retard["ForwardTrack x"].Slider/-1, values.different.retard["ForwardTrack y"].Slider, values.different.retard["ForwardTrack z"].Slider)
					end
				elseif (  b.z > a.z ) then
					if ( b.x > a.x ) then
						j.Character.UpperTorso.Waist.C0= CFrame.new(values.different.retard["ForwardTrack x"].Slider, values.different.retard["ForwardTrack y"].Slider, values.different.retard["ForwardTrack z"].Slider)
...
varechka:Element('ToggleKeybind', 'inf respawn', {}, function()
	game.ReplicatedStorage.Events:FindFirstChild("Spawnme"):FireServer()
end)
varechka:Element('ToggleKeybind', 'desync', {}, function()
...
kbot:Element("Dropdown", "knifebot type", {options = {"standart", "semi lag", "lag", "P2000test"}})
...
aimbot:Element('Button', 'Buy AK47', {}, function()
    local gun = "AK47" 
    local givefunc
...
aimbot:Element('Button', 'Buy G3SG1', {}, function()
    local gun = "G3SG1"
    local givefunc
    for _, v in pairs(getgc()) do
    local parentScript = rawget(getfenv(v), "script")
...
aimbot:Element('Button', 'Buy R8', {}, function()
	-- pasted from google
	local gun = "R8"
	local givefunc
	for _, v in pairs(getgc()) do
...
aimbot:Element('Button', 'Buy Galil', {}, function()
	local gun = "Galil"
	local givefunc
	for _, v in pairs(getgc()) do
	local parentScript = rawget(getfenv(v), "script")
...
local loopkillplr = {}
		...
		exploits:Element("Dropdown", "Player", {options = loopkillplr, Amount = 33})
		...
		exploits:Element("ToggleKeybind", "Loop kill", nil, function(tbl)
			if tbl.Toggle then
				...
				if _G.Disable1 then step1:Disconnect() return end
					if Players[values.rage["exploits"].Player.Dropdown].Character and Players[values.rage["exploits"].Player.Dropdown].Team ~= LocalPlayer.Team and Players[values.rage["exploits"].Player.Dropdown].Character:FindFirstChild("UpperTorso") then
						local oh1 = Players[values.rage["exploits"].Player.Dropdown].Character.Head
						local oh2 = Players[values.rage["exploits"].Player.Dropdown].Character.Head.CFrame.p + Vector3.new(0, 0/0, 0) 
						local oh3 = "CT Knife"
...
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "custom", "KarKarich", "win", "nyan", "baimware"}})
world:Element('Dropdown', 'killsound', {options = {'none', 'killPacalets', 'test', 'test1'}})
...
client:Element("Jumbobox", "gun modifiers", {options = {"recoil", "spread", "ammo", "penetration", "firerate"}})
...
chat:Element("Dropdown", "kill say type", {options = {"off", "standard", "cripbot", "rosense", "dead-inside", "troll J.", "femware", "cris-paste", "Vaderhaxx", "MEMZ"}})
	...
	--[[ if values.rage.fakelag["ping spike"].Toggle and values.rage.fakelag["ping spike"].Active then  
		...
		if values.something.varechka["kill all mode"].Dropdown == "hard" and values.something.varechka["J kill all"].Toggle and values.something.varechka["J kill all"].Active and LocalPlayer.Character:FindFirstChild("Head") and LocalPlayer.Character:FindFirstChild("Gun") then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("Head") then
					local oh1 = Player.Character.Head
					local oh2 = Player.Character.Head.Position + Vector3.new(0, 0/0, 0) 
					local oh3 = Client.gun.Name
					local oh4 = 400
					local oh5 = LocalPlayer.Character.Gun
					local oh8 = 1000
		...
		if values.something.varechka["kill all mode"].Dropdown == "random" and values.something.varechka["J kill all"].Toggle and values.something.varechka["J kill all"].Active and LocalPlayer.Character:FindFirstChild("Head") and LocalPlayer.Character:FindFirstChild("Gun") then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("Head") then
					local oh1 = Player.Character.Head
					local oh2 = Player.Character.Head.Position + Vector3.new(0, 0/0, 0) 
					local oh3 = AllWeapons[math.random(14,#AllWeapons)]
					local oh4 = 986897687678676
					local oh5 = nil
					local oh6 = nil
		...
		if values.something.varechka["kill all type"].Dropdown == "new" and values.something.varechka["pasted kill all"].Toggle and values.something.varechka["pasted kill all"].Active and LocalPlayer.Character:FindFirstChild("UpperTorso") and LocalPlayer.Character:FindFirstChild("Gun") then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("UpperTorso") then
					...
					local oh2 = Player.Character.UpperTorso.Position + Vector3.new(0, 0/0, 0) 
					local oh3 = "Banana"
					local oh4 = 666666
					local oh5 = Client.Gun
					local oh6 = Vector3.new()
					...
					local oh18 = Vector3.new()
                    local oh19 = Vector3.new()
					local oh20 = Vector3.new(0, 0, 0)
                    local oh21 = Vector3.new()
					local oh22 = Vector3.new()
                    local oh23 = Vector3.new()
					local oh24 = Vector3.new(0, 0, 0)
					...
					local oh86 = Vector3.new()
                    local oh87 = Vector3.new()
					local oh88 = Vector3.new(0, 0, 0)
                    local oh89 = Vector3.new()
					local oh90 = Vector3.new()
                    local oh91 = Vector3.new()
					local oh92 = Vector3.new(0, 0, 0)
                    local oh93 = Vector3.new()
					local oh94 = Vector3.new()
                    local oh95 = Vector3.new(0, 0, 0)
					local oh96 = Vector3.new()
                    local oh97 = Vector3.new()
					local oh98 = Vector3.new()
                    local oh99 = Vector3.new()
		...
		if values.something.varechka["better kill all"].Toggle and values.something.varechka["better kill all"].Active and LocalPlayer.Character:FindFirstChild("UpperTorso") and LocalPlayer.Character:FindFirstChild("Gun") then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("UpperTorso") then
					...
					local oh4 = 2000
					local oh5 = nil
					local oh6 = Vector3.new(0, 0, 0) 
					local oh7 = Vector3.new(2, 2/2, 2) 
					local oh8 = 7777777
					local oh9 = false
					local oh10 = true
					local oh11 = Vector3.new(0, 0, 0)
					...
					if Player.Team ~= LocalPlayer.Team and Player.Character and Player.Character:FindFirstChild('Humanoid') and not Client.DISABLED and Player.Character:FindFirstChild('Humanoid').Health > 0 and Player.Team ~= 'TTT' and not Player.Character:FindFirstChildOfClass('ForceField') and Player ~= LocalPlayer then
						if Player.Status.Alive.Value then
							if Client.gun:FindFirstChild("Melee") and values.something.kbot["knifebot"].Toggle then
								...
								local Ray = Ray.new(Origin, (Player.Character.HumanoidRootPart.Position - Origin).unit * (values.something.kbot["distance"].Slider * 55))
								...
								if values.something.kbot["knifebot type"].Dropdown == "P2000test" then
									local Arguments = {
										[1] = Hit,
										[2] = Hit.Position,
										[3] = "P2000TEST",
	...
	if not values.rage.aimbot.enabled.Toggle and values.legit.aimbot["triggerbot"].Toggle and values.legit.aimbot["triggerbot"].Active and not TriggerDebounce then
		...
		if values.something.varechka['C4 kill all'].Toggle and values.something.varechka['C4 kill all'].Active and LocalPlayer.Character:FindFirstChild('UpperTorso') and LocalPlayer.Character:FindFirstChild('Gun') then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild('UpperTorso') then
					local oh1 = Player.Character.Head
					local oh2 = Player.Character.Head.CFrame.p + Vector3.new(0, 0/0, 0) 
					local oh3 = "C4"
					local oh4 = 4096
					local oh5 = nil
					local oh6 = Vector3.new()
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal('Value'):Connect(function(current)
	...
	sound.Parent = game:GetService('SoundService')
	sound.SoundId = values.visuals.world.killsound.Dropdown == 'killPacalets' and 'rbxassetid://10807717726' or values.visuals.world.killsound.Dropdown == 'test' and 'rbxassetid://10736876085' or values.visuals.world.killsound.Dropdown == 'test1' and 'rbxassetid://10807714305'
	sound.Volume = values.visuals.world["sound volume"].Slider
	sound.PlayOnRemove = true
	sound:Destroy()
	lastcurrent = LocalPlayer.Status.Kills.Value
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	...
	if values.misc.chat["kill say type"].Dropdown == "standard" then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "cripbot-paste", false, "Innocent", false, true)
	end
 end)
 LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	if current == 0 then return end
...
Camera.ChildAdded:Connect(function(obj)
	while true do
		...
		if table.find(values.misc.client["gun modifiers"].Jumbobox, "ammo") then
			Client.ammocount = 30000000000
			Client.ammocount2 = 30000000000
		end
	end
end)
Camera.ChildAdded:Connect(function(obj)
...
LocalPlayer.Cash:GetPropertyChangedSignal("Value"):Connect(function(cash)
	if values.misc.client["infinite cash"].Toggle and cash ~= 9000000000 then
